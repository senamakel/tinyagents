...
trait FutureNode:
    std::future::Future<Output = Result<NodeResult<StatePatch>>> + std::marker::Send + 'static
{
}

impl<T> FutureNode for T where
    T: std::future::Future<Output = Result<NodeResult<StatePatch>>> + std::marker::Send + 'static
{
}

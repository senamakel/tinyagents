...
async fn graph_builder_validates_topology_and_exports_metadata() {
    ...
    let start_to_end = GraphBuilder::<i32, i32>::overwrite()
        .add_edge(START, tinyagents_graph::END)
        .compile()
        .unwrap_err();
    assert!(
        start_to_end
    ...
    let builder = GraphBuilder::<i32, i32>::overwrite()
        ...
        .add_edge("join", "cmd")
        .with_command_destinations("cmd", ["join", tinyagents_graph::END])
    ...
    let graph = builder.compile().unwrap().with_checkpointer(Arc::new(
        tinyagents_graph::InMemoryCheckpointer::<i32>::new(),
    ));

...
fn pipe_reducer()
-> ClosureStateReducer<Pipe, Patch, impl Fn(Pipe, Patch) -> tinyagents_harness::Result<Pipe>> {
    ...
    GraphBuilder::<Pipe, Patch>::new()
        ...
        .add_node("dispatch", |_s: Pipe, _c: NodeContext| async move {
            Ok(NodeResult::Command(
                tinyagents_graph::Command::goto(["work_a", "work_b"]).with_update(Patch {
        ...
        .add_conditional_edges(
            ...
            [("again", "dispatch"), ("done", tinyagents_graph::END)],
        )
        .compile()

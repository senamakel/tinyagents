...
fn pipeline() -> tinyagents_graph::CompiledGraph<i64, i64> {
...
async fn a_failed_node_emits_run_failed_and_a_failed_status() {
    let graph = GraphBuilder::<i64, i64>::overwrite()
        .add_node("boom", tinyagents_graph::failing_node("kaboom"))
        .set_entry("boom")
        .set_finish("boom")
        .compile()
    ...
    // returns an error.
    let recorder = tinyagents_graph::GraphEventRecorder::new();

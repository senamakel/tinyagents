...
//!
//! These types add **durability** to the live [`crate::stream`] event
...
/// tool call (see [`crate::SubAgentNode`]) — so per-node success/failure
...
/// [`tinyagents_harness::store::JsonlAppendStore`] for a local durable journal or
/// [`tinyagents_harness::store::InMemoryAppendStore`] for deterministic tests.
#[derive(Clone, Debug)]
...
/// [`crate::stream::GraphEventSink::flush`] blocks until the durable log
...
/// live transport (for example a [`crate::stream::CollectingSink`]) so a

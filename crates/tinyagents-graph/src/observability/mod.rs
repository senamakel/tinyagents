...
//!
//! The live [`crate::stream`] layer emits transient [`GraphEvent`]s into
...
//!   [`crate::GraphRunStatus`], with an [`InMemoryGraphStatusStore`].
...
//!   harness [`LangfuseClient`](tinyagents_harness::observability::LangfuseClient)
...
//!
//! [`CompiledGraph`](crate::CompiledGraph) can be wired to write to a
//! status store and a journal through its builder-style
//! [`with_status_store`](crate::CompiledGraph::with_status_store) and
//! [`with_event_journal`](crate::CompiledGraph::with_event_journal)
...
//! [`AppendWorker`](tinyagents_harness::observability)): `emit` never blocks the
...
use tinyagents_harness::store::AppendStore;
...
fn poisoned<E: std::fmt::Display>(
    what: &str,
    err: E,
) -> tinyagents_harness::error::TinyAgentsError {
    tinyagents_harness::error::TinyAgentsError::Validation(format!("{what} lock poisoned: {err}"))
}

...
use tinyagents_harness::ids::{ExecutionStatus, GraphId, NodeId, RunId};
use tinyagents_harness::store::InMemoryAppendStore;
...
fn graph_obs(run: &str, offset: u64, step: usize, event: GraphEvent) -> GraphObservation {
    GraphObservation {
        event_id: tinyagents_harness::ids::EventId::new(format!("g-evt-{offset}")),
...
async fn store_backed_journal_round_trips() {
    ...
    let obs = GraphObservation {
        event_id: tinyagents_harness::ids::EventId::new("e0"),
        run_id: RunId::new("r0"),
...
fn status_on_thread(run_id: &str, thread_id: &str, status: ExecutionStatus) -> GraphRunStatus {
    ...
    s.thread_id = Some(tinyagents_harness::ids::ThreadId::new(thread_id));
    s
}

...
use crate::reducer::{OverwriteStateReducer, StateReducer};
    ...
    Update: Send + 'static,
{
    ...
    pub fn new() -> Self {
        Self {
            graph_id: GraphId::new(format!("graph-{}", tinyagents_harness::ids::next_seq())),
            name: None,
            nodes: HashMap::new(),
            edges: HashMap::new(),
    ...
    /// branch finished first. See [`crate::CompiledGraph`] for the full
    ...
    /// from the [`crate::Command`] a node emits — but they let the
    /// [topology export](crate::export) draw and validate the set of
    ...
    /// surfaced as [`crate::NodeInfo::kind`] in the export.
    ...
    /// verbatim into [`crate::NodeInfo::metadata`].

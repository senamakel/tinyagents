...
struct TestAgentRegistry {
    agents: HashMap<String, Arc<dyn HarnessAgent>>,
}

impl TestAgentRegistry {
    fn register(&mut self, agent: Arc<dyn HarnessAgent>) {
...
/// with `answer`.
fn registry_with_agent(name: &str, answer: &str) -> Arc<dyn AgentRegistry> {
    ...
    let mut registry = TestAgentRegistry::default();
    registry.register(HarnessSubAgent::new(subagent).into_dyn());
    Arc::new(registry)
}
...
) -> crate::CompiledGraph<String, String> {
    let _ = agent;
...
async fn unregistered_agent_fails_with_capability() {
    let registry: Arc<dyn AgentRegistry> = Arc::new(TestAgentRegistry::default());
    let node = SubAgentNode::<String, String>::from_fns(
        "missing",
        |s: &String| SubAgentInput::prompt(s.clone()),
        |out: SubAgentOutput| out.text,

...
//!
//! ```
//! use tinyagents_graph::{GraphBuilder, NodeResult, START, END};
//! use tinyagents_graph::export::{to_json, to_mermaid};
//!
...
fn node_parts<'a, I>(
    ids: I,
    is_command: impl Fn(&tinyagents_harness::ids::NodeId) -> bool,
    meta: &std::collections::HashMap<tinyagents_harness::ids::NodeId, crate::builder::NodeMeta>,
) -> Vec<NodePart>
where
    I: IntoIterator<Item = &'a tinyagents_harness::ids::NodeId>,
{
    ids.into_iter()
        .map(|id| {
            let m = meta.get(id);
...
/// [`to_json`]. Returns [`tinyagents_harness::error::TinyAgentsError::Serialization`] on
/// malformed input.

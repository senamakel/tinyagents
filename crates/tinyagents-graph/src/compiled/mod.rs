...
//! with [`crate::GraphBuilder::with_parallel`], a step with more than one
...
use crate::command::{Command, Interrupt, NodeResult, RouteTarget};
...
/// process restarts.
///
/// Delegates to [`tinyagents_harness::ids::new_checkpoint_id`]: a resumed thread
...
fn next_checkpoint_id() -> String {
    tinyagents_harness::ids::new_checkpoint_id()
        .as_str()
        .to_string()
}
...
/// [`Checkpoint::to_metadata`](crate::Checkpoint::to_metadata) so a
...
/// [`crate::Send`] packet carries `Some(arg)`. Multiple activations may
...
/// [`Interrupt::node`](crate::command::Interrupt::node): a subgraph node
...
impl<State, Update> CompiledGraph<State, Update> {
    ...
    /// [`GraphBuilder::with_name`](crate::GraphBuilder::with_name).
    ...
    /// policy opts in via
    /// [`RetryPolicy::with_backoff_sleep`](tinyagents_harness::retry::RetryPolicy::with_backoff_sleep).
    ///
    ...
    /// frame (and the parent/child [`RunTree`](crate::RunTree)) names the
    ...
    /// into a [`crate::observability::GraphObservation`] (stamped with the
    ...
    pub fn with_event_journal(
        mut self,
        journal: Arc<dyn crate::observability::GraphEventJournal>,
    ) -> Self {
    ...
    pub fn with_status_store(
        mut self,
        status_store: Arc<dyn crate::observability::GraphStatusStore>,
    ) -> Self {

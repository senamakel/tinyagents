...
async fn exit_durability_persists_only_terminal_checkpoint() {
    use crate::checkpoint::DurabilityMode;
...
impl Checkpointer<i32> for SlowCheckpointer {
    ...
    ) -> tinyagents_harness::error::Result<Option<crate::checkpoint::Checkpoint<i32>>> {
    ...
    async fn list(
        &self,
        thread_id: &str,
    ) -> tinyagents_harness::error::Result<Vec<crate::checkpoint::CheckpointMetadata>> {
        self.inner.list(thread_id).await
    }

    async fn list_threads(&self) -> tinyagents_harness::error::Result<Vec<String>> {
        self.inner.list_threads().await
    }

    async fn delete_thread(&self, thread_id: &str) -> tinyagents_harness::error::Result<()> {
    ...
    async fn delete_checkpoints(
        ...
        thread_id: &str,
        ids: &[String],
    ) -> tinyagents_harness::error::Result<usize> {
...
fn two_step_graph() -> crate::GraphBuilder<i32, i32> {
    GraphBuilder::<i32, i32>::overwrite()
        .add_node("a", |s, _c: NodeContext| async move {
            Ok(NodeResult::Update(s + 1))
        })

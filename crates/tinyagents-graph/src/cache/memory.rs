...
/// One stored entry: the value plus its absolute expiry (if any).
type Entry = (serde_json::Value, Option<Instant>);

/// An in-memory [`TaskCache`]: a mutex-guarded map honoring per-entry TTL
/// on read. Expired entries are dropped lazily when read; `clear` (or
/// dropping the cache) is what otherwise reclaims them.
#[derive(Default)]
pub struct InMemoryTaskCache {
    entries: Mutex<HashMap<TaskCacheKey, Entry>>,
}
...
impl InMemoryTaskCache {
    ...
    fn lock(&self) -> Result<std::sync::MutexGuard<'_, HashMap<TaskCacheKey, Entry>>> {
        self.entries
            .lock()

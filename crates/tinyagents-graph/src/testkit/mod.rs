...
//! [`testkit`](tinyagents_harness::testkit) (model/tool doubles + trajectories):
...
//! | [`fanout_node`] | Emits one [`Send`](crate::Send) per arg (fanout) |
...
use crate::builder::{NodeContext, NodeFuture};
use crate::command::{Command, Interrupt, NodeResult, Send as SendPacket};
use crate::compiled::{CompiledGraph, GraphExecution, StateSnapshot};
...
/// [`END`](crate::END) to end a branch. An empty queue makes every
...
/// [`Send`](crate::Send) per argument, so `target` is scheduled once for
...
/// [`shared_subgraph_node`](crate::shared_subgraph_node)).
///
...
{
    crate::subgraph::shared_subgraph_node(child)
}
...
impl GraphEventRecorder {
    ...
    /// [`CompiledGraph::with_event_sink`](crate::CompiledGraph::with_event_sink).
...
impl StreamCollector {
    ...
    /// [`GraphEvent::NodeCompleted`](crate::GraphEvent)).
...
impl<State> GraphAssertions<'_, State> {
    ...
    pub fn completed(&self) -> &Self {
        ...
        assert_eq!(
            ...
            tinyagents_harness::ids::ExecutionStatus::Completed,

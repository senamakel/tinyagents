...
impl ChatModel<()> for ProviderFailingModel {
    async fn invoke(
        ...
        *self.attempts.lock().unwrap() += 1;
        Err(tinyinference_core::Error::Provider(Box::new(
            tinyinference_core::model::ProviderError {
                provider: "test-provider".to_string(),
                status: Some(self.status),
                retryable: self.retryable,
                retry_after_ms: None,
                message: "boom".to_string(),
                ..tinyinference_core::model::ProviderError::default()
            },
        )))
    }
}
...
impl Middleware<(), ()> for DeltaRecorder {
    ...
    async fn on_model_delta(
        ...
        _ctx: &mut RunContext<()>,
        _state: &(),
        delta: &mut tinyinference_core::model::ModelDelta,
    ) -> Result<()> {
...
impl ChatModel<()> for ToolCapturingModel {
    async fn invoke(
        ...
        next.ok_or_else(|| {
            tinyinference_core::Error::Validation("no scripted response left".into())
        })
    }
}

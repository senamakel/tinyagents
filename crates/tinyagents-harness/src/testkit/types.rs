...
//!
//! Every public struct and enum in `crate::testkit` is declared here.
...
/// A [`tinyinference_llm::model::ChatModel`] that yields a scripted sequence of
/// [`ModelStreamItem`]s, exercising the real streaming pipeline deterministically.
///
/// Each call to [`tinyinference_llm::model::ChatModel::stream`] replays the same
/// scripted items, and [`tinyinference_llm::model::ChatModel::invoke`] returns the
/// merged response those items fold into (via
/// [`tinyinference_llm::model::StreamAccumulator`]), so the mock behaves
/// consistently on both the streaming and unary paths.
///
/// # Example
///
/// ```rust
/// # use tinyagents_harness::testkit::StreamingMock;
...
/// A [`tinyinference_llm::model::ChatModel`] that sleeps for a fixed delay before
/// replying, used to deterministically trigger the agent loop's per-model-call
/// wall-clock timeout.
///
/// Both [`tinyinference_llm::model::ChatModel::invoke`] and
/// [`tinyinference_llm::model::ChatModel::stream`] first
...
pub struct SlowModel {
    /// How long [`tinyinference_llm::model::ChatModel::invoke`] sleeps before
    /// replying.
...
/// A [`tinyinference_llm::model::ChatModel`] that returns pre-loaded responses in
...
///
/// ```rust
/// # use tinyagents_harness::testkit::DeterministicClock;
...
///
/// ```rust
/// # use tinyagents_harness::testkit::DeterministicIds;
...
/// # use tinyagents_harness::events::AgentEvent;
/// # use tinyagents_harness::ids::{RunId, CallId};
/// let events = vec![

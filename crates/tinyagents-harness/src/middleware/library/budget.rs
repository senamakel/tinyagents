...
impl BudgetMiddleware {
    ...
    pub fn with_pricing(
        mut self,
        pricing: std::collections::HashMap<String, crate::cost::ModelPricing>,
    ) -> Self {
    ...
    fn price(&self, response: &ModelResponse) -> crate::cost::CostTotals {
        let Some(usage) = response.usage else {
            return crate::cost::CostTotals::default();
        };
        let Some(name) = response.resolved_model.as_ref().map(|r| r.name.as_str()) else {
            return crate::cost::CostTotals::default();
        };
        match self.pricing.get(name) {
            Some(pricing) => crate::cost::estimate_cost(pricing, &usage),
            None => crate::cost::CostTotals::default(),
        }
    }
}

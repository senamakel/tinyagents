...
//! sibling modules:
//!
//! - [`crate::lexer`] produces [`SpannedToken`]s from source text.
//! - [`crate::parser`] turns tokens into a [`Program`] AST.
//! - [`crate::compiler`] lowers a [`Program`] into one [`Blueprint`]
//!   per graph and wires blueprints into the runtime graph.
//!
//! The source AST node types (`Program`, `GraphDecl`, `NodeDecl`, …) live in
//! [`crate::ast`] and are re-exported here for back-compatibility.

use serde::{Deserialize, Serialize};

// Re-export the source AST so existing `crate::types::{Program, …}`
...
// The source [`Span`] type lives in [`crate::span`] and is re-exported
// here so existing `crate::types::Span` paths keep resolving.
...
pub struct Blueprint {
    ...
    ///
    /// Populated by [`crate::compiler::compile_with_provenance`] (and
    /// the [`crate::testkit`] helpers). The plain [`compile`] path

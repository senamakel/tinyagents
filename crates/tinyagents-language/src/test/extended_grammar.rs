...
fn extended_blueprint_round_trips_through_serde() {
    ...
    let back: crate::types::Blueprint = serde_json::from_str(&json).unwrap();
    assert_eq!(bp, back);
}
...
fn extended_kinds_bind_against_a_resolver() {
    ...
    let resolver = CapabilityResolver::from_lists(["default".to_string()], std::iter::empty())
        ...
        .with_node_kinds(
            crate::capability_resolver::DEFAULT_NODE_KINDS
                .iter()
                .map(|k| k.to_string()),
        );
...
fn steering_block_is_rejected_at_compile_time_until_it_is_enforced() {
    ...
    match err {
        tinyagents_harness::error::TinyAgentsError::Compile(message) => {

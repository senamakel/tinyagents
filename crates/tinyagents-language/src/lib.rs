...
//! exact same `tinyagents-graph` and [`tinyagents_harness`] runtime as
//! hand-written Rust, yet can only *reference* capabilities — never define or
//! execute code — so an agent-emitted plan is parsed, validated, and bound
//! against a registry before it ever runs.
//!
...
//! It compiles into the same `tinyagents-graph` and [`tinyagents_harness`]
//! structures as hand-written Rust through a fixed pipeline:
//!
//! ```text
...
pub use capability_resolver::{
    CapabilityKind, CapabilityResolver, CapabilitySource, bind_capabilities,
    bind_capabilities_with_registry,
};
pub use compiler::{compile, compile_source, compile_with_provenance};
...
pub use span::Span;
pub use testkit as language_testkit;
pub use types::*;

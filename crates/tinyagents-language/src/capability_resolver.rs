...
//!
//! Lives beside [`crate::resolver`] (the spanned-diagnostic
//! resolution path) since both implement the same binding policy;
//! [`crate::resolver::Resolver`] wraps a [`CapabilityResolver`]
...
/// Capability namespaces understood by registry-backed language validation.
...
pub enum CapabilityKind {
    ...
    /// Callable tools.
    Tool,
    /// Registered graph blueprints.
    Graph,
    /// Conditional routers.
    Router,
    /// State reducers.
    Reducer,
    /// Executable sub-agents.
    Agent,
    /// Registered scripts.
    Script,
...
///
/// Registry implementations live in a higher-level crate and implement this
/// trait without making the language compiler depend on a concrete registry.
pub trait CapabilitySource {
    /// Returns canonical names and aliases registered for `kind`.
    fn names(&self, kind: CapabilityKind) -> Vec<String>;
}
...
/// [`crate::resolver::Resolver`] paths — routes through it so they
/// cannot drift.
...
impl CapabilityResolver {
    ...
    /// [`bind_capabilities_with_registry`].
    pub fn from_registry(registry: &impl CapabilitySource) -> Self {
        let collect = |kind| registry.names(kind).into_iter().collect();
        Self {
            models: collect(CapabilityKind::Model),
            tools: collect(CapabilityKind::Tool),
            subgraphs: collect(CapabilityKind::Graph),
            routers: collect(CapabilityKind::Router),
    ...
    /// [`crate::resolver::Resolver`] paths from drifting: a new node
    ...
    /// [`crate::resolver::Resolver`] paths stay in lockstep.
...
pub fn bind_capabilities_with_registry(
    blueprint: &Blueprint,
    registry: &impl CapabilitySource,
) -> Result<()> {

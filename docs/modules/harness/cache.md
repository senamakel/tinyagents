...
- When `RunPolicy::empty_response_retries` is enabled, a completion with no
  visible text or tool call is not cached, and an older blank cache hit is
  treated as a miss. The retry must reach the provider rather than replay the
  blank response under the same key.

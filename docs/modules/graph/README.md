...
The graph runtime does not know or care where a graph came from.
...
- OpenHuman PR #4261 is useful as a product reference for keeping a generic
  ...
  HITL, observability events, and RPC run control.
- TinyAgents should preserve the ergonomic Rust builder surface from these
  ...
  inspected by UIs, driven from a host orchestrator, and tested with
  deterministic state/channel snapshots.

## Design Stance

The graph module is the stable execution contract below all graph authoring
surfaces. A graph may be built by Rust code, assembled by a host orchestration
session, or restored from durable state. Once compiled, those origins must converge on the same immutable
...
The graph runtime must not accept unchecked model-generated topology or
executable code.

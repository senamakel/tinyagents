...
| Graph goals, todo board, task dispatch, subagents, observability | integration tests plus orchestration's `e2e_graph_subagent_node.rs` | Covered; delegation durability remains unit-only |
...
| Subagent orchestration | `crates/tinyagents-orchestration/tests/e2e_*subagent*.rs` | Direct invocation, tools, reuse, limits, graph nodes, and hosted orchestration covered |

...
**M15. `session` hard-enables `harness/sqlite`** (`session/Cargo.toml:16`) and `orchestration` depends on `session`, so any workspace build compiles bundled SQLite; the "opt-in" story in `docs/sdk-gaps/durability.md` §5 only holds for standalone harness consumers. Consider a `sqlite` feature on session/orchestration too. Effort S.

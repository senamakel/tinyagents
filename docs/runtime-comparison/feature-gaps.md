...
| A5 | Agent loop compiled to the graph runtime so checkpoints, interrupts, streaming and time travel apply to the loop itself; step iteration (`agent.iter()`) | LG (`create_agent` → `StateGraph`), PA (`iter`, `UserPromptNode`…) | LG §4, PA §4 | `subagent_node`, `docs/modules/harness/state-graph.md` (promised) | RT harness+graph | 2 | partial (loop reuses the same phase node bodies via `GraphLoopDriver`, but that driver is not itself a resumable `CompiledGraph` checkpoint -- no graph-level checkpoint/resume/time-travel for the agent loop itself) |
...
| F5 | Deferred / background model responses (`DeferredHandle`, `stop_reason: deferred`) | pi | pi §4.9 | none | RT tinyinference | 3 | open |
| F6 | Request/response escape hatches (`on_payload`, `on_response`, `fetch` injection) | pi | pi §5 | decorators in `tinyinference-llm/model/decorators.rs` | RT tinyinference | 3 | open |

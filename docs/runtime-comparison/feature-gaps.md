...
| A7 | `interrupt_before/after` selectors, `interrupt(response_schema)`, graceful drain (`RunControl`) | LG | LG §3.8 | documented in `interrupts.md`, `mark_interrupt` export-only, `CancellationToken` | RT graph | 3 | shipped (phase-4, pending merge) |
...
| D5 | Durable task memoisation inside a node (`durable_task(ctx, key, fut)`) so side effects before an interrupt are not repeated | LG (functional API) | LG §3.9 | `PendingWrite`, `interrupts.md` "must use idempotency keys" | RT graph | 3 | shipped (phase-4, pending merge) |
